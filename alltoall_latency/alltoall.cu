...
#define CALL_ALLTOALL(TYPENAME, TYPE, TG_PRE, THREADGROUP, THREAD_COMP,        \
                      ELEM_COMP)                                               \
  __global__ void test_##TYPENAME##_alltoall_call_kern##THREADGROUP(           \
      nvshmem_team_t team, TYPE *dest, const TYPE *source, int nelems,         \
      int mype, int iter) {                                                    \
    int i;                                                                     \
                                                                               \
    if (!blockIdx.x && (threadIdx.x < THREAD_COMP) && (nelems < ELEM_COMP)) {  \
      for (i = 0; i < iter; i++) {                                             \
        nvshmem##TG_PRE##_##TYPENAME##_alltoall##THREADGROUP(team, dest,       \
                                                             source, nelems);  \
      }                                                                        \
    }                                                                          \
  }
...
int alltoall_calling_kernel(nvshmem_team_t team, void *dest, void *source,
                            int mype, int max_elems, cudaStream_t stream,
                            void **h_tables) {
  int status = 0;
  ...
  for (num_elems = 1; num_elems < max_elems; num_elems *= 2) {
    ...
    status = nvshmemx_collective_launch(
        (const void *)test_int32_alltoall_call_kern_block, num_blocks,
        nvshm_test_num_tpb, args_1, 0, stream);
    if (status != NVSHMEMX_SUCCESS) {
      fprintf(stderr, "shmemx_collective_launch failed %d \n", status);
      exit(-1);
  ...
  if (!mype) {
    print_table("alltoall_device", "32-bit-thread", "size (Bytes)", "latency",
                "us", '-', h_size_array, h_thread_lat, i);
    print_table("alltoall_device", "32-bit-warp", "size (Bytes)", "latency",
                "us", '-', h_size_array, h_warp_lat, i);
    print_table("alltoall_device", "32-bit-block", "size (Bytes)", "latency",
                "us", '-', h_size_array, h_block_lat, i);
  }
  ...
  for (num_elems = 1; num_elems < max_elems; num_elems *= 2) {
    ...
    status = nvshmemx_collective_launch(
        (const void *)test_int64_alltoall_call_kern_block, num_blocks,
        nvshm_test_num_tpb, args_1, 0, stream);
    if (status != NVSHMEMX_SUCCESS) {
      fprintf(stderr, "shmemx_collective_launch failed %d \n", status);
      exit(-1);
  ...
  if (!mype) {
    print_table("alltoall_device", "64-bit-thread", "size (Bytes)", "latency",
                "us", '-', h_size_array, h_thread_lat, i);
    print_table("alltoall_device", "64-bit-warp", "size (Bytes)", "latency",
                "us", '-', h_size_array, h_warp_lat, i);
    print_table("alltoall_device", "64-bit-block", "size (Bytes)", "latency",
                "us", '-', h_size_array, h_block_lat, i);
  }
...
int main(int argc, char **argv) {
  ...
  char *value = NULL;
  // size needs to hold psync array, source array (nelems) and dest array
  // (nelems * npes)
  ...
  CUDA_CHECK(cudaMemcpyAsync(d_source, h_source,
                             (sizeof(DATATYPE) * num_elems * npes),
                             cudaMemcpyHostToDevice, cstrm));
  CUDA_CHECK(cudaMemcpyAsync(d_dest, h_dest,
                             (sizeof(DATATYPE) * num_elems * npes),
                             cudaMemcpyHostToDevice, cstrm));

  alltoall_calling_kernel(NVSHMEM_TEAM_WORLD, (void *)d_dest, (void *)d_source,
                          mype, max_elems, cstrm, h_tables);
